...
def teach_dash(request):
    ...
    if 'UserName' in request.session:
        try:
            ...
            print(getDetails)
            studentCount = Students_Model.objects.all().count()
            studentSubmittedAssignmentCount = Assignemnt_Submissions.objects.all().count()
            pendingAssignment = Assignemnt_Submissions.objects.filter(
                assignment_submittion_status="notViewed").count()
            pendingLimitforLoop = Assignemnt_Submissions.objects.filter(
                assignment_submittion_status="notViewed")[:5:1]
            context = {
                'userName': getDetails.teach_UserName,
                'studentCount': studentCount,
                'studentSubmittedAssignmentCount': studentSubmittedAssignmentCount,
                'pendingAssignment': pendingAssignment,
                'pendingLimitforLoop': pendingLimitforLoop
    ...
    if 'UserName' in request.session:
        try:
            ...
            if request.method == 'POST':
                ...
                saveStudent = Students_Model(stdnt_Name=frm_stdnt_FLNM, stdnt_Roll=frm_stdnt_Roll, stdnt_Branch=frm_stdnt_Branch, stdnt_DOB=frm_stdnt_DOB, stdnt_Gender=frm_stdnt_Gender,
                                             stdnt_Email=frm_stdnt_Email, stdnt_Mobile=frm_stdnt_Mobile, stdnt_UserName=frm_stdnt_USRName, stdnt_PWD=frm_stdnt_PWD, stdnt_is_Active=frm_stdnt_active).save()
                print(saveStudent)
...
def teach_del_stdnt(request):
    if 'UserName' in request.session:
        try:
            try:
                DelStudentData = Students_Model.objects.get(
                    stdnt_id=request.GET.get('id'))
    ...
    if 'UserName' in request.session:
        try:
            ...
            if request.method == 'POST':
                ...
                saveNewAssignment = Assignments(assignment_DIR=frm_assign_DIRNAME, assignment_QN=frm_assign_QN, assignment_Marks=frm_assign_Marks,
                                                assignment_Last_Date=frm_assign_LastDate, assignment_FileType=frm_assign_FileType, assignment_status=True).save()
...
def teach_lst_assign(request):
    ...
    if 'UserName' in request.session:
        try:
            ...
            plag(AssignmentListData[0].assignment_DIR,
                 AssignmentListData[0].assignment_FileType)
...
def teach_edit_assign(request):
    if 'UserName' in request.session:
        ...
        if request.method == "POST":
            ...
            upDateStudent = Assignments.objects.filter(assignment_id=request.POST.get('id')).update(
                assignment_QN=frm_assign_QN, assignment_Marks=frm_assign_marks, assignment_Last_Date=frm_assign_Last_Date, assignment_status=frm_assignment_status)
            print(upDateStudent)
...
def teach_del_assign(request):
    if 'UserName' in request.session:
        ...
        try:
            DeleteAssignment = Assignments.objects.get(
                assignment_id=request.GET.get('id'))
...
def teach_view_assignment(request):
    ...
    if 'UserName' in request.session:
        ...
        if request.method == "GET":
            if request.GET.get('id'):
                ...
                assignmentData = Assignemnt_Submissions.objects.filter(
                    assignment_submittion_id=request.GET.get('id'))
                studentData = Students_Model.objects.filter(
                    stdnt_Roll=assignmentData[0].assignment_submited_Roll)
                print(studentData[0])
        ...
        if request.method == "POST":
            ...
            print('Assignment Updated Posted')
            upDateStudent = Assignemnt_Submissions.objects.filter(assignment_submittion_id=request.POST.get('id')).update(
                assignment_submited_Marks=frm_marks, assignment_submittion_status=frm_status, assignment_submittion_review=frm_review)
            print(upDateStudent)
    ...
    if 'UserName' in request.session:
        if request.GET.get('id'):
            ...
            AssignmentListData = Assignemnt_Submissions.objects.filter(
                assignment_Model_ID=request.GET.get('id'))

            AssignmentDirVar = Assignments.objects.filter(
                assignment_id=request.GET.get('id'))
            # print(AssignmentDirVar[0].assignment_FileType)
            Matched_Content = plag(
                AssignmentDirVar[0].assignment_DIR, AssignmentDirVar[0].assignment_FileType)
...
def tech_add_notices(request):
    if 'UserName' in request.session:
        try:
            ...
            if request.method == 'POST':
                POST = request.POST
                notice_added_user = getDetails.teach_UserName
                ...
                print(SaveNotice)
                return HttpResponseRedirect('teach_add_notice?suc=added')

            return render(request, 'teacher/teach_add_notice.html', context)
        except Teachers_Model.DoesNotExist:
            return HttpResponseRedirect('login')
    else:
        return HttpResponseRedirect('login')
...
def teach_view_notices(request):
    if 'UserName' in request.session:
        try:
            getDetails = Teachers_Model.objects.get(
                teach_UserName=request.session['UserName'])
            ...
            return render(request, 'teacher/teach_view_notices.html', context)
        except Teachers_Model.DoesNotExist:
            return HttpResponseRedirect('login')
    else:
        return HttpResponseRedirect('login')
...
def teach_view_notice(request):
    if 'UserName' in request.session:
        try:
            ...
            return render(request, 'teacher/teach_view_notice.html', context)
        except Teachers_Model.DoesNotExist:
            return HttpResponseRedirect('login')
    else:
        return HttpResponseRedirect('login')
...
def plag(PassDir, PassType):
    ...
    fileType = PassType
    studnt_Submitted_Files = [
        doc for doc in os.listdir(dir) if doc.endswith(fileType)]
    studnt_Submitted_Files_DT = [
        open(dir+"/"+File).read() for File in studnt_Submitted_Files]
