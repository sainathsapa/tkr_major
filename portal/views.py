...
def reg_tech(request):
    if request.method == 'POST':
        ...
        saveTeacher = Teachers_Model(teach_Name=frm_teachName, tech_Gender=frm_tech_Gender, teach_Email=frm_teach_Email,
                                     teach_Mobile=frm_teach_Mobile, teach_UserName=frm_teach_UserName, teach_PWD=frm_teach_PWD).save()
...
def login(request):
    if request.method == 'POST':
        ...
        if(login_type == 'teacher'):
            ...
            try:
                userDeat = Teachers_Model.objects.get(teach_UserName=request.POST.get(
                    'login_UserName'), teach_PWD=request.POST.get('login_PWD'))
        ...
        if(login_type == 'student'):
            ...
            try:
                userDeat = Students_Model.objects.get(stdnt_UserName=request.POST.get(
                    'login_UserName'), stdnt_PWD=request.POST.get('login_PWD'))
        ...
        if(login_type == 'accounts'):

            try:
                userDeat = accounts_usr.objects.get(acc_UserName=request.POST.get(
                    'login_UserName'), acc_PWD=request.POST.get('login_PWD'))
                request.session['Acc_UserName'] = userDeat.acc_UserName
                return HttpResponseRedirect('acc_dashboard')
